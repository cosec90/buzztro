...
function VALIDATE($username,$password){
	...
	$sql= "SELECT * FROM `buzztro_admin` WHERE `adm_username`='$username' AND `adm_password`='$password' AND `adm_status`='Active'";
...
function CHECK_USER($username){
	include '../Admin_config/connection.php';
	$sql= "SELECT `adm_username` FROM `buzztro_admin` WHERE `adm_username`='$username' AND `adm_status`='Active'";
	$result=mysqli_query($conn,$sql);
	$count=mysqli_num_rows($result);
...
function FORGET_PASS($admin,$newpass){
 	include '../Admin_config/connection.php';
  	...
  	$sql= "UPDATE `buzztro_admin` SET `adm_password`='$newpass' WHERE `adm_username` = '$admin'";
  	$result=mysqli_query($conn,$sql);
